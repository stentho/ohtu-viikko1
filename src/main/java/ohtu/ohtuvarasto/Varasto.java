...
public class Varasto {
    ...
    public Varasto(double tilavuus, double alkuSaldo) { // kuormitetaan
        konstruoiTilavuus(tilavuus);
        konstruoiSaldo(alkuSaldo);
    }

    public void konstruoiTilavuus (double tilavuus) {
        if (tilavuus > 0.0) {
            this.tilavuus = tilavuus;
        } else { // virheellinen, nollataan
            this.tilavuus = 0.0;  // => käyttökelvoton varasto
    ...
    public void konstruoiSaldo (double alkuSaldo) {
        if (alkuSaldo < 0.0) {
            this.saldo = 0.0;
    ...
    public void lisaaVarastoon(double maara) {
        if (maara < 0) { // virhetilanteessa voidaan
            return;       // tällainen pikapoistuminenkin!
        }
        if (maara <= paljonkoMahtuu()) { // omia aksessoreita voi
